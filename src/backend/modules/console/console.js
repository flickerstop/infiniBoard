...
module.exports = class {
    ...
    static test(string){
        if(isTest){
            ...
            this.saveLog("TEST STRING: " + string);
        }
    }
    ...
    static saveLog(string){
        ...
        fs.appendFile(logFileName, outputString, function (err) {
            if (err){
                console.error("append file",err);
            }
        });
    ...
    static guild(guildID,string){
        console.log(timestamp() + blue(guildID) + "--> " + string);
        this.saveLog(blue(guildID) + "--> " + string);
    }
